...
async function printETHBalance({ address, name }) {
  console.log(`${name} FTM balance is`, web3.utils.fromWei(await web3.eth.getBalance(address)))
}
...
async function generateMerkleProof(deposit) {
  ...
  let  allEvents = await smashnado.getPastEvents('Deposit', { fromBlock: 	0, toBlock: 	'latest' })  
...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  ...
  if (relayerURL) {
    ...
    try {
      ...
      if (netId === 250) {
        console.log(`Transaction submitted through the relay. View transaction on fantomscan https://ftmscan.com/`)
      } else {
    ...
    await smashnado.methods.withdraw(proof, ...args).send({ from: senderAccount, value: refund.toString(), gas: 1e6 })
      .on('transactionHash', function (txHash) {
        if (netId === 250) {
          console.log(`View transaction on ftmscan https://ftmscan.com/`)
        } else {
...
async function w_getdeposits(_currencyStr, _amountStr) {
  try {
    const currencyStr = _currencyStr.toLowerCase()
    ...
    if(Array.isArray(allEvents) && allEvents.length >0) {
      let deposithtml = ''
      allEvents.reverse()
      allEvents.length > 8 ? allEvents = allEvents.slice(1,9) : allEvents
      
      allEvents.forEach(event=> { deposithtml += '<div class="col-sm-6"><div class="bg-text">' + conert_timestamp(event.returnValues.timestamp) + '</div></div>'})
      $('#his_deposit').html(deposithtml)
      $('#deposit_count').html(allEvents.length + ' equal user deposits' )
    } else {
      $('#his_deposit').html('<p>No deposits</p>')
      $('#deposit_count').html('')
    }
  } catch (error) {
    setLoadingCircle(false)
 ...
 */
async function init({ rpc, noteNetId, currency = 'ftm', amount = '1' }) {
  ...
  try {
    if (web3 == undefined) {
      ...
      contractJson = await (await fetch('build/contracts/FTMSmashnado.json')).json()
  ...
  erc20 = currency !== 'ftm' ? new web3.eth.Contract(erc20ContractJson.abi, tokenAddress) : {}
}

async function main() {
  const instance = { currency: 'ftm', amount: '1' }
  //await init(instance)
  ...
  window.deposit = async (_currencyStr, _amountStr) => {
    ...
    if ( parseFloat(senderBalance) < parseFloat(_amountStr) ) {
      w_display_notification('Insufficient FTM Balance in your wallet to deposit.', 'error')
      return false;
    }
    ...
    try {
      ...
      if (noteString) {
        ...
        resultStr += 'Sender account FTM balance is ' + web3.utils.fromWei(await web3.eth.getBalance(senderAccount)) +'<br/>'
